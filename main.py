...
def check_streamable(file_name: str, mime_type: str = ""):
    ...
    if ext in [".mp4", ".m4v"]:
        return True, "✅ আপনি এই ভিডিওটি সরাসরি ওয়েব প্লেয়ারে দেখতে পারবেন।"
    else:
        return False, f"⚠️ এটি {ext.upper()} ফাইল। ব্রাউজারে সমস্যা হলে নিচে 'Open in External Player' বাটন ব্যবহার করুন।"
...
async def watch_player(
    ...
    # Intent URLs for External Players
    vlc_url = f"vlc://{default_stream_url}"
    mx_url = f"intent:{default_stream_url}#Intent;package=com.mxtech.videoplayer.ad;type=video/*;end"
    ...
    <head>
        ...
        <style>
            * {{ box-sizing: border-box; margin: 0; padding: 0; }}
            ...
            .vlc {{ background-color: #ff8800; }}
            .mx {{ background-color: #0088cc; }}
    ...
    <body>
        ...
        <div class="player-container">
            <div class="quality-control-bar">
                <label>SS Anime Box Player</label>
                <select class="quality-select" id="qualitySelector" onchange="switchStream(this.value)">
                    {quality_options_html}
                </select>
            </div>
        ...
        <div class="ext-player-box">
            <h3>ভিডিও প্লে হতে সমস্যা হলে বাইরের প্লেয়ারে দেখুন:</h3>
            <div class="btn-group">
                <a href="{vlc_url}" class="ext-btn vlc">Open in VLC</a>
                <a href="{mx_url}" class="ext-btn mx">Open in MX Player</a>
                <a href="{URL}/download/{chat_id}/{message_id}/{file_name}" class="ext-btn download">Direct Download</a>
            </div>
        </div>
...
async def handle_file_stream(chat_id: int, message_id: int, file_name: str, request: Request, is_download: bool = False):
    ...
    if range_header:
        # Proper Byte Range Calculation for Fast Seeking
        ...
        async def fast_ranged_streamer():
            # Chunking logic for Hydrogram/Pyrogram
